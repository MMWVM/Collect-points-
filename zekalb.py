...
dylar8 = """**مرحبا بك في قسم تحويل النقاط
...
dylar9 = """**مرحبا بك في قسم تحويل النقاط
...
dylar10 = """ze1 = TelegramClient(StringSession(session), api_id, api_hash)
...
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
        order = await event.reply(dylar1)
...
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
        order = await event.reply(dylar8)
...
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownerhson_id :
        order = await event.reply(dylar9)
